...
test_image_flair_path = TRAIN_DATASET_PATH + "\BraTS20_Training_355\BraTS20_Training_355_flair.nii"
test_image_t1_path = TRAIN_DATASET_PATH + "\BraTS20_Training_355\BraTS20_Training_355_t1.nii"
test_image_t1ce_path = TRAIN_DATASET_PATH + "\BraTS20_Training_355\BraTS20_Training_355_t1ce.nii"
test_image_t2_path = TRAIN_DATASET_PATH + "\BraTS20_Training_355\BraTS20_Training_355_t2.nii"
test_image_seg_path = TRAIN_DATASET_PATH + "\BraTS20_Training_355\BraTS20_Training_355_seg.nii"

#Loading an nifti-file as an array adn plotting max pixel value 
test_image_flair=nib.load(test_image_flair_path).get_fdata()
print(test_image_flair.max())

#rescaling is required
#Scalers are applied to 1D so let us reshape and then reshape back to original shape. 
test_image_flair=scaler.fit_transform(test_image_flair.reshape(-1, test_image_flair.shape[-1])).reshape(test_image_flair.shape)
#now max value is 1, everything is now scaled between 0 and 1
print(test_image_flair.max())


#Now the same with the other images
test_image_t1=nib.load(test_image_t1_path).get_fdata()
test_image_t1=scaler.fit_transform(test_image_t1.reshape(-1, test_image_t1.shape[-1])).reshape(test_image_t1.shape)

test_image_t1ce=nib.load(test_image_t1ce_path).get_fdata()
test_image_t1ce=scaler.fit_transform(test_image_t1ce.reshape(-1, test_image_t1ce.shape[-1])).reshape(test_image_t1ce.shape)

test_image_t2=nib.load(test_image_t2_path).get_fdata()
test_image_t2=scaler.fit_transform(test_image_t2.reshape(-1, test_image_t2.shape[-1])).reshape(test_image_t2.shape)


#Mask
test_mask=nib.load(test_image_seg_path).get_fdata()
test_mask=test_mask.astype(np.uint8)

print(np.unique(test_mask))  #0, 1, 2, 4 (Need to reencode to 0, 1, 2, 3)
test_mask[test_mask==4] = 3  #Reassign mask values 4 to 3
print(np.unique(test_mask)) #now the values have changed to 0, 1, 2, 3


#Showing a slice of the selected dataset
import random
...
combined_x = np.stack([test_image_flair, test_image_t1ce, test_image_t2], axis=3) #240,240,155,3 --> 3 represents flai, t1ce and t2
...
test_mask = to_categorical(test_mask, num_classes=4) #from 128x128x128 to 128x128x128x4, as we got 4 classes
...
for img in range(len(t2_list)):   #Using t1_list as all lists are of same size
    ...
    val, counts = np.unique(temp_mask, return_counts=True) #return_counts returns number of pixel, which got the labels
